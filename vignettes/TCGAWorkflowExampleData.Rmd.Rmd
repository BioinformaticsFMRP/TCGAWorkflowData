...
This package provides a dataset for those wishing to try out the workflow
in Bioconductor: TCGAWorkflow. The data in this package are a subset of the TCGA data
for LGG (Lower grade glioma ) and GBM (Glioblastoma mMltiforme) samples.

# Loading the data

```{r, fig.show='hold'}
library(TCGAWorkflowExampleData)
data(TCGAWorkflowExampleData)
TCGAWorkflowExampleData
```
